-- A copy of the lobby module, because the two places are separate bundles and neither
-- can see the other's files. Worth having on both sides rather than only in the lobby:
-- skill points come from dungeon XP, so you level up mid-run and would otherwise be
-- sitting on unspent points for the rest of it. Both the remote and the counter exist
-- here, checked against this place's own dump.
