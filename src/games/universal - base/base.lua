...
--[[
	Rank command API - lets a Discord-ranked user (Premium/Privileged/Owner) run a
	command on a lower-ranked player, from in-game or from Discord. See backend/README.md.

	Security note: there is deliberately no secret embedded in this script. A secret
	shared by every Vain install would let anyone who reads the public script
	impersonate a ranked user and forge commands as them. Instead:
	  - pollKey is generated locally per-install and only ever proves "I'm still the
	    same client that's been polling as this uid" (trust-on-first-use) - it can't
	    grant a rank, only steal/block a queued command for this account specifically.
	  - commandKey is the real credential: issued once by Discord after linking, never
	    shipped in the script, and is the only thing the server accepts as proof of
	    "this request really is from a ranked account."
]]
local rankapi = {
	baseUrl = 'https://vain-api.vain-api.workers.dev',
	commands = {
		kick = function(args)
			task.spawn(function()
...
if isfile('vain/profiles/rankcommandkey.txt') then
	rankapi.commandKey = readfile('vain/profiles/rankcommandkey.txt')
end

local function rankapiGet(path, params)
	local query = {}
	...
	local suc, res = pcall(function()
		return httpService:JSONDecode(game:HttpGet(rankapi.baseUrl..path..'?'..table.concat(query, '&')))
	end)
	return suc and res or nil
end

-- Shared by the clipboard chat-trigger below and the Settings > General textbox
-- (src/guis/new/gui.lua) - either way the key is never typed into real chat.
function rankapi.setCommandKey(key)
	if not key or key == '' then return end
	writefile('vain/profiles/rankcommandkey.txt', key)
	rankapi.commandKey = key
	notif('Vain', 'Rank key saved.', 5)
end

-- Reads the personal command key from the clipboard rather than chat, so it never
-- gets typed into (and broadcast through) real chat text.
local function rankapiSetKeyFromClipboard()
	if not getclipboard then
		notif('Vain', 'Your executor does not support getclipboard - cannot safely read the key.', 10, 'alert')
		return
	end

	local key = getclipboard()
	if not key or key == '' then
		notif('Vain', 'Clipboard is empty. Copy your key from Discord first, then run ;rank key.', 10, 'alert')
		return
	end

	rankapi.setCommandKey(key)
end

-- Issues a command against targetName, if the local player outranks them. Server-side
-- is the sole source of truth for both ranks - this call only fails fast on obvious
-- local problems (no key set, bad target name), everything else happens remotely.
-- Exposed as vain.Libraries.rankapi.issueCommand so the GUI (Settings > General) can
-- call it too, not just the chat-trigger below.
function rankapi.issueCommand(commandName, targetName, reason)
	if not rankapi.commandKey then
		notif('Vain', 'Set your personal key first: copy it from Discord, then run ;rank key.', 10, 'alert')
		return
	end

	local suc, targetUserId = pcall(function()
		return playersService:GetUserIdFromNameAsync(targetName)
	end)
	if not suc or not targetUserId then
		notif('Vain', 'Could not find a Roblox user named "'..targetName..'".', 10, 'alert')
		return
	end

	local res = rankapiGet('/command', {
		issuer_uid = lplr.UserId,
		target_uid = targetUserId,
		command = commandName,
		args = httpService:JSONEncode({reason or ''}),
		key = rankapi.commandKey
	})

	if not res then
		notif('Vain', 'Rank command failed: could not reach the API.', 10, 'alert')
	elseif res.error then
		notif('Vain', 'Rank command failed: '..res.error, 10, 'alert')
	end
end

vain.Libraries.rankapi = rankapi

-- Long-polls for a command targeting the local player and runs it on arrival. One held
-- request at a time - see backend/README.md for the request-budget reasoning.
local function rankapiStartPolling()
	task.spawn(function()
		repeat
			local res = rankapiGet('/poll', {
				uid = lplr.UserId,
				key = rankapi.pollKey
			})

			if res and res.status == 'command' then
				local handler = rankapi.commands[res.command]
				if handler then
					handler(res.args or {})
				end
			elseif not res then
				task.wait(5) -- API unreachable, back off instead of hammering it
			end
...
-- One-time welcome notif showing the local player's tier (0 = Free, included). Reuses
...
-- whitelist.commands trigger accepts), so it's briefly visible to others in the
-- server - the key itself never is, since it's read from the clipboard, not typed.
vain:Clean(lplr.Chatted:Connect(function(message)
	local args = message:split(' ')
	if args[1] ~= ';rank' or not args[2] then return end

	if args[2]:lower() == 'key' then
		rankapiSetKeyFromClipboard()
		return
	end

	if not args[3] then return end
	local commandName = args[2]:lower()
	local targetName = args[3]
	table.remove(args, 1)
	table.remove(args, 1)
	table.remove(args, 1)

	if rankapi.commands[commandName] then
		rankapi.issueCommand(commandName, targetName, table.concat(args, ' '))
	end
end))
...
run(function()
	...
	function whitelist:announce(text)
		...
		if success then
			return sendToast({
				toastTitle = text,
				iconImage = getcustomasset('vain/assets/new/vain.png'),
				swipeUpDismiss = true,
		...
		icon.Image = getcustomasset('vain/assets/new/vain.png')
